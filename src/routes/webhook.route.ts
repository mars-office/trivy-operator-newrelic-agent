...
import {
  Finding,
  NewRelicVulnerabilityReport,
} from "../models/new-relic-vulnerability-report";
...
webhookRouter.post("/api/webhook", async (req: Request, res: Response) => {
  try {
    ...
    if (
      !data ||
      !data.kind ||
      data.kind.toLowerCase() !== "vulnerabilityreport"
    ) {
      res.status(400).send({ success: false, error: "Invalid payload!" });
      return;
    }
  ...
  query {
    actor {
      entitySearch(query: "domain='INFRA' AND type='CONTAINER' AND tags.k8s.clusterName='${
        data.metadata.labels.env
      }' AND tags.k8s.namespaceName='${
      data.metadata.namespace
    }' AND tags.k8s.${data.metadata.labels[
      "trivy-operator.resource.kind"
    ].toLowerCase()}Name='${
      data.metadata.labels["trivy-operator.resource.name"]
    }' AND tags.k8s.containerName='${
      data.metadata.labels["trivy-operator.container.name"]
    }'") {
        count
        results {
          entities {
  ...
  `;
    ...
    const nerdGraphReply = await axios.post<NerdGraphResponse>(
      process.env.NERD_GRAPH_URL!,
      {
    ...
    const newRelicPayload: NewRelicVulnerabilityReport = {
      findings: data.report?.vulnerabilities?.map(
        (v) =>
          ({
            entityType: "Container",
            entityGuid:
              nerdGraphReply.data.data.actor.entitySearch.results.entities[0]
                .guid,
    ...
    const newRelicReply = await axios.post<NewRelicVulnerabilityResponse>(
      process.env.NEW_RELIC_VULN_URL!,
      newRelicPayload,
      {
        headers: {
    ...
    if (!newRelicReply.data.success) {
      res
        .status(400)
        .send({ success: false, error: newRelicReply.data.errorMessage });
      return;
    }
